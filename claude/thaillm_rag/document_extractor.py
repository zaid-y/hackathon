...
class ThaiTextChunker:
    ...
    Respects Thai word boundaries and attempts to preserve sentence structure.
    """
    ...
    def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[ExtractedDocument]:
        ...
        # Split by sentences first if enabled
    ...
    def _split_sentences(self, text: str) -> List[str]:
        """Split text into sentences (Thai + English)"""
        # Thai sentence endings: ฯ, 。, ？, ！ + English . ? !
        # Also handle Thai-specific patterns
    ...
    def _split_long_sentence(self, sentence: str, base_metadata: Dict, start_index: int) -> List[ExtractedDocument]:
        ...
        for i in range(0, len(tokens), self.config.chunk_size - self.config.chunk_overlap):
            ...
            chunk_text = "".join(chunk_tokens)  # Thai doesn't use spaces between words
            chunks.append(self._make_chunk(chunk_text, base_metadata, start_index + len(chunks)))
    ...
    def _make_chunk(self, content: str, base_metadata: Dict, chunk_index: int) -> ExtractedDocument:
        """Create ExtractedDocument with chunk metadata"""
